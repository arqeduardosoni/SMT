    ...
    return <div key={screen} style={{minHeight:"100vh",background:IB,color:TX,fontFamily:F.ios,position:"relative"}}>
      ...
      <div style={{padding:"0 0 90px"}}>
        ...
        <div style={{padding:"0 18px"}}>
          ...
          <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",gap:6,height:100,margin:"6px 0 6px"}}>{bars.map((v,i)=><div key={i} style={{flex:1,height:Math.max(6,(v/maxP)*100)+"%",borderRadius:"5px 5px 0 0",background:i===bars.length-1?C.cyan:LIME,opacity:v?1:0.3,transformOrigin:"bottom",animation:`growUp .7s ${i*0.07}s ease both`}}/>)}</div>
      ...
      <div style={{padding:"0 0 90px"}}>
        ...
        <div style={{padding:"0 18px"}}>
          ...
          <div style={{background:IC,borderRadius:18,padding:"16px 14px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:6}}>{cells.map((d,i)=>{const ds=d?`${mp}-${String(d).padStart(2,"0")}`:null;const isDone=ds&&done.has(ds);const isTod=ds===todStr;return <div key={i} style={{aspectRatio:"1",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",background:isDone?LIME:(d?IC2:"transparent"),border:isTod?`2px solid ${C.cyan}`:"none",animation:d?`statPop 0.4s ${i*0.012}s backwards`:"none"}}>{d&&<span style={{fontFamily:F.ios,fontSize:13,fontWeight:isDone?800:500,color:isDone?LIMED:MU}}>{d}</span>}</div>;})}</div>
          </div>
